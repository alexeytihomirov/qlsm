"""demo-management's private copy of the SFTP-to-a-managed-host plumbing for
reading an instance's demo dir.

Used to be ui/instance_demo_transport.py in qlsm core, shared with the
qlmatch-packer addon (which needed the same "resolve instance -> host, open
a key-authenticated SFTP session, list/fetch named files" mechanics for its
own qlmatch-specific listing/download). Duplicated instead of kept shared
once it became clear the addon system has no way to declare or enforce a
dependency between addons -- see ui/stats_hub.py's old docstring (now gone
with it) and addons/README.md's "Where the line falls" for the full
reasoning, which applies here identically. This is this addon's own code
again, same as before the original split. This copy and qlmatch-packer's are
