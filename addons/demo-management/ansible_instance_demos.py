...
to ansible-playbook for a two-line remote operation. The SFTP plumbing lives
in this addon's own instance_demo_transport.py -- qlmatch-packer, which needs
the same "open a session to this instance's demo dir" mechanics for its own
format-specific reads, has its own private copy rather than sharing this one
(see that module's docstring for why).

Lives inside the demo-management addon (moved from ui/task_logic/ in core).
The addon's own JWT-protected panel calls straight into this module -
