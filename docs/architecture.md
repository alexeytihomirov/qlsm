...
*   **Plugin Pool as Source of Truth (2026-08-23):** `ql-assets/data/minqlx-plugins/` is the **authoritative** copy of every custom minqlx/minqlxtended plugin QLSM ships (`match_restore`, `tournament_access`, `chat_rcon`, `stream_telemetry_unified`, `demo_native_autorecord`, `lobby`, etc.) — it's what `setup_host.yml` deploys as the host baseline, what `ui/task_logic/backup_files.py` backs up, and what `ui/plugin_manifest.py` reads `*.ql-plugin.json` metadata from regardless of where a `.py` file physically lives (pool entry always wins over a preset/instance sidecar). All plugin logic edits happen in the pool, not in `configs/presets/_builtin/default/scripts/` (the builtin "default" `ConfigPreset`) or any instance's own copy. Historically the *default preset's* copy could silently drift from the pool since nothing synced file *content* between them (manifest lookup already preferred the pool, but `.py` code did not) — `ui/plugin_pool_sync.py` (`flask sync-plugin-pool`, `--check` for CI) now mirrors the pool onto the builtin default preset's `scripts/` dir, and `tests/test_plugin_pool_preset_sync.py` fails the suite if they diverge again. The one deliberate exception is `highfps.py`/`highfps_hook.so`, a togglable per-preset LD_PRELOAD hook, not a pool plugin. Line endings for both trees are pinned to LF via `.gitattributes` so a CRLF/LF-only diff can't masquerade as content drift. Separately (also 2026-08-23), the minqlxtended C-patch chain (item-events, item-respawn, native demo capture, redis-pool, set-position) was vendored from `ql-server-core`'s `qlds-scripts/patch-minqlxtended-*.py` into `ql-assets/patches/minqlxtended/` and wired into engine build (`ansible/playbooks/tasks/build_engine_hook.yml`, `_engine_apply_minqlxtended_qlhub_patches`) — pending live verification on a real instance, `ql-server-core` remains the engine C-patch reference and is not archived yet.
*   **Demo Capture Pipeline: Standardize on Native, eBPF Not Ported (2026-08-23):** Two independent per-player demo capture mechanisms exist in the monorepo. The older one is the eBPF pipeline (`ql-server-core/addons/server-demo/` — a uprobe on `SV_Netchan_Encode` feeding `ql-demo-recorder`'s separate Go `demo-writer` process over a per-instance Unix socket, armed/disarmed via local HTTP). The newer one is native: `minqlxtended.demo_arm()`/`demo_disarm()` calling straight into the vendored `demos.c`/`demo_slot.c`/UDT-cutter code that already ships in QLSM's own C-patch chain (see the plugin-pool bullet above) — no separate process, no socket, and its hook (`SV_SendMessageToClient`, pattern-scanned by minqlxtended's own `Hook()`) is resilient to engine updates in a way the eBPF path's hand-pinned `SV_Netchan_Encode` address/SHA256 is not. Decision: **QLSM standardizes on native-demo going forward; the eBPF sidecars (`demo-hook`, `ql-demo-recorder`) are not being ported here.** This is not yet a decommission of eBPF — it remains `ql-server-core`'s only live-validated demo capture path (still armed in that repo's own parallel, not-yet-archived deploy story) because native-demo has three concrete gaps, one now closed: (1) **closed 2026-08-25** — `demo_native_autorecord.py`/`demo_native_manifest.py` moved from `ql-server-core/addons/native-demo/minqlx/` into `ql-assets/data/minqlx-plugins/` (pool) + builtin default preset `scripts/`, each with its own `.ql-plugin.json` (`demo_native_autorecord.ql-plugin.json` real manifest with `qlx_nativeDemoRecordEnabled`; `demo_native_manifest.ql-plugin.json` the usual "helper, do not enable separately" stub, same pattern as `match_restore_util`/`telemetry_unified_sched`); unit tests ported to `tests/test_demo_native_manifest.py`. `ql-server-core`'s own copy is now the frozen one (see that repo's `docs/BUSINESS.md` plugin-pool-SoT note); (2) `demo_arm`/`demo_disarm` have only ever been build/link-verified (Woodpecker CI compiles the patched engine and checks the native re-exports) — no one has confirmed a live match actually produces a playable per-POV `.dm_91` + `.qlmatch` on a real instance; (3) the demo-editor consumer has never been exercised against the `.qlmatch` format. Once (2)-(3) close, eBPF can retire from `ql-server-core`'s own deploy story too (that repo's `docs/BUSINESS.md` "не архивируется целиком" gate #2 — non-plugin addon sidecars).
...
2. "Demos" instance action → `GET /api/instances/<id>/demos` → `list_instance_demos()` opens a key-authenticated SFTP session straight to the host (paramiko, same target resolution as `service_runtime.py`'s runtime probe / `rcon_transport.py`'s live rcon — no Ansible) and `listdir_attr()`s that directory, filtering to `DEMO_FILENAME_RE` and returning `{name, size, mtime}` per file, newest first. This is a read of what's actually on disk, not a claim from a plugin or cvar state, so it doubles as the observable result of a manual demo-recording test with the current minqlxtended build.
1a. After every finished native-demo match (`demo_match_finalized`), `demo_native_autorecord.py` launches the external **qlmatch-packer** (`ql-assets/data/qlmatch-packer/`, deployed to `/home/ql/qlmatch-packer/` with Node 22 by `ansible/playbooks/tasks/sync_qlmatch_packer.yml` — wired into both `setup_host.yml` and the "Update Plugins" `update_common_plugins.yml`) as a separate process. It validates the shared POV window (refusing to pack an empty/short overlap), builds `manifest.json` + zip (STORE) per the `.qlmatch` contract, renders the operator's filename template (`qlx_qlmatchNameTemplate`), and copies the result to every configured rclone target (`qlx_qlmatchRcloneTargets`; rclone install/config on the host is a manual operator step). Both cvars are editable from the Plugins tab via the plugin's `.ql-plugin.json` manifest and travel through server.cfg like any other cvar. Per-match log: `<demo_dir>/<match_id>.packer.log`. It also generates a merged-replay sidecar, `<demo_dir>/<match_id>_<map>.replay.json.gz` (`qlmatch-to-replay.mjs`, never inside the `.qlmatch` zip), which the same Demos modal lists and downloads too — it's what the dashboard's demo/replay view and `!restorecp qlmatch` read. Demo protocol parsing reuses the production `ql-stream-tools/live-overlay/lib/qldemo` parser, vendored verbatim under `qlmatch-packer/vendor/qldemo/` (refresh: `sync-vendor.sh`). If the packer or node is missing on a host, the plugin falls back to its old in-process zip build (no templating/delivery).
2a. The same modal supports downloading: `GET /api/instances/<id>/demos/download?filename=...` fetches one file via `fetch_instance_demos()` (an SFTP `open()`/`read()` per filename over the same paramiko session, missing files collected instead of failing the batch) and streams it back in-memory; `POST /api/instances/<id>/demos/download-batch` does the same for a checkbox-selected set of filenames and zips the results in-memory before streaming. Filenames are validated against `DEMO_FILENAME_RE` before either ever reaches SFTP, since they're concatenated into a remote filesystem path.
