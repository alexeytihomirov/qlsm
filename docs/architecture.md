...
2. A best-effort local safety snapshot (`build_backup_zip_bytes()` written to `backup_snapshots/pre-restore-<timestamp>.qlsmbak`) is captured before anything destructive happens — recovery-of-last-resort only, never exposed in the UI.
