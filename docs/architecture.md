...
*   **Demo Capture: Native minqlxtended, No Separate Sidecar:** Per-player demo capture uses minqlxtended's own native `demo_arm()`/`demo_disarm()` (straight into the vendored `demos.c`/`demo_slot.c`/UDT-cutter code shipped in the qlhub C-patch chain — see the `minqlxtended-patched` runtime), armed/disarmed from `demo_native_autorecord.py` on `game_countdown`/`game_end` and finalized on `demo_match_finalized`. No separate process, no socket, and its hook point (`SV_SendMessageToClient`, pattern-scanned rather than address-pinned) is resilient to engine updates.
...
2. "Demos" instance action → `GET /api/instances/<id>/demos` → `list_instance_demos()` opens a key-authenticated SFTP session straight to the host (paramiko) and lists that directory, filtering to a demo-filename pattern and returning `{name, size, mtime}` per file, newest first. This is a read of what's actually on disk, not a claim from a plugin or cvar state.
3. After every finished native-demo match (`demo_match_finalized`), `demo_native_autorecord.py` launches the external **qlmatch-packer** (`ql-assets/data/qlmatch-packer/`, deployed to the host with Node 22 by `ansible/playbooks/tasks/sync_qlmatch_packer.yml`, wired into `setup_host.yml`) as a separate process. It validates the shared POV window (refusing to pack an empty/short overlap), builds `manifest.json` + a STORE zip per the `.qlmatch` contract, renders the operator's filename template (`qlx_qlmatchNameTemplate`), and copies the result to every configured rclone target (`qlx_qlmatchRcloneTargets`; rclone install/config on the host is a manual operator step). Both cvars are editable from the Plugins tab via the plugin's manifest. Per-match log: `<demo_dir>/<match_id>.packer.log`. It also generates a merged-replay sidecar, `<demo_dir>/<match_id>_<map>.replay.json.gz` (`qlmatch-to-replay.mjs`, never inside the `.qlmatch` zip), which the same Demos modal lists and downloads too. Demo protocol parsing is a vendored qldemo parser under `qlmatch-packer/vendor/qldemo/` (refresh via `sync-vendor.sh`). If the packer or Node is missing on a host, the plugin falls back to its old in-process zip build (no templating/delivery).
4. The same modal supports downloading: `GET /api/instances/<id>/demos/download?filename=...` fetches one file via `fetch_instance_demos()` (an SFTP read per filename over the same paramiko session, missing files collected instead of failing the batch) and streams it back in-memory; `POST /api/instances/<id>/demos/download-batch` does the same for a checkbox-selected set of filenames and zips the results in-memory before streaming. Filenames are validated against a demo-filename pattern before either ever reaches SFTP, since they're concatenated into a remote filesystem path.
5. The external Bearer-token API mirrors a filtered view of the same listing at `/api/v1/instances/<id>/matches` (just `.qlmatch` packs, each flagged with `has_replay`/`replay_name` derived from the pack's own `manifest.json`, not from string-editing the filename — the operator-configurable name template can render it to anything) plus `/matches/download` and `/matches/replay` for downloading either file by name.
