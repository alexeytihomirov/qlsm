...
*   **Automation Runner:** Represents the logic within the RQ worker responsible for executing the automation tools, now refactored into the `ui/task_logic/` package:
    ...
    * **Plugin Manifests & Repositories:**
        ...
        * `ui/plugin_repositories.py`: The one component that makes **outbound HTTP requests to an operator-supplied address** — it fetches a remote `qlsm-plugins.json`, downloads individual plugin files into the shared pool, from where Ansible ships them to every host, and — for the overwrite prompt's Diff window — fetches a read-only copy of a single plugin so the repository version can be compared against the pool copy without writing anything. A `github.com` URL is rewritten to its `raw.githubusercontent.com` base. Downloaded filenames are constrained to a bare `^[A-Za-z0-9_-]+\.py$` allowlist at both parse and write time. A repo's single `qlsm-plugins.json` can carry each plugin's cvars/commands inline; download writes them into the pool as the plugin's `.ql-plugin.json`, so `ui/plugin_manifest.py` reads them unchanged. A fresh install starts with one repository already listed (`ui/plugin_repository_cli.py`, seeded by the entrypoint on first-run database init only). The parse rules this module applies are re-implemented client-side in `frontend-react/src/utils/pluginManifestValidation.js`, which backs the repository card's **Edit & Export Manifest** modal — a local authoring aid that hands back a `qlsm-plugins.json` to commit to the repository, with no endpoint and no write to `PluginRepository.manifest_json` (a verbatim fetch cache that every sync overwrites); see [Plugin Manifest Validation Mirrors](development_guidelines.md#plugin-manifest-validation-mirrors).
    * **Supporting Modules:**
