...
-   **Archive script (`/usr/local/bin/qlsm-archive-serverlogs.sh`):** For each numeric `/home/ql/qlds-<port>/` directory (non-numeric suffixes such as `qlds-base` are skipped), appends new entries from `journalctl -u qlds@<port>.service --cursor-file=/var/lib/qlsm/logcursor-<port>` into `<dir>/serverlogs/server.log`, then runs `logrotate -s /var/lib/qlsm/logrotate.state /etc/qlsm/qlds-serverlogs.conf`. An instance's first sighting seeds its cursor at the journal tail (`journalctl -n0 --cursor-file=...`, emitting nothing) rather than exporting existing history, so pre-existing journal content is never imported — start-fresh, not backfilled. The cursor file is removed on instance delete (`manage_qlds_service.yml`).
...
-   **Self-provisioning:** `fetch_server_log_archive.yml` (the read playbook) checks for `/etc/systemd/system/qlsm-archive-serverlogs.timer` and re-runs `tasks/server_log_archiving.yml` if it's missing, so a host provisioned before this feature existed serves an archive read instead of erroring. The gate tests for the timer unit specifically — the *last* file the shared task list deploys — rather than the script, because gating on the script would read "already installed" and skip re-provisioning if a prior partial run failed after the script landed but before the logrotate policy did.
-   **Read path (`fetch_server_log_archive.yml`):** runs the archive script once more before reading, but only when the requested `filename` is `server.log` — flushing an archive mid-read risks a concurrent rotation deleting the very file being read. Content is transported via `ansible.builtin.fetch` with `become: false` (required for the fast SFTP path rather than a base64 slurp) into a local temp file, which `fetch_instance_server_log()` (`ui/task_logic/ansible_server_log_archives.py`) reads and returns; the temp directory is always cleaned up.
