...
QLSM builds and runs one of three runtimes per host: **minqlx** (`MinoMino/minqlx`, the original), **minqlxtended** (`tjone270/minqlxtended`, a hard fork built unpatched), and **minqlxtended-patched** (`alexeytihomirov/minqlxtended` — our fork of the same upstream with the former qlhub patch chain landed as real commits, built without any patch scripts; see `ansible/playbooks/tasks/build_engine_hook.yml`; this is what host "germany" actually runs). minqlx and vanilla minqlxtended have no plugin compatibility in either direction; minqlxtended-patched deliberately shares minqlx's plugin pool and shared directory instead of getting its own (see below). `ui/runtime.py` is the single source of truth for every value that differs between them — nothing else in the backend or the Terraform roots hardcodes a runtime-specific path or pin (the Ansible playbooks re-derive the same values themselves from the `runtime` extra-var, since Ansible cannot import this module directly). Its API:
...
| Git repo / pinned commit | `MinoMino/minqlx` @ `fbdd915185337791d8e209dc4b686a1ee60d3721` | `tjone270/minqlxtended` @ `97fbe6715a4802545aa7eca741d11e2486a306a4` | `tjone270/minqlxtended` @ floating `HEAD` (**no pin** — known open risk, see `ui/runtime.py`) |
...
**Host creation:** `POST /api/hosts` accepts an optional `runtime` field (default `minqlx`, `400` if unrecognized) and stores it on `Host.runtime`, which is never writable again after creation — moving a host to another runtime means saving a preset and deploying a new host. The Terraform OS variables (`os_name`, `os_family`) and the standalone/self-host remote-OS-detection Python-version check are both derived from the chosen runtime, so a minqlxtended/minqlxtended-patched host is provisioned on Ubuntu 24.04 or refused if the target machine can't meet the Python 3.12 floor. `setup_host.yml` re-asserts the same Python floor server-side (`ansible_python_version is version('3.12', '>=')`) before building, so a manual playbook run against an unsuitable host fails fast rather than compiling against the wrong interpreter.
...
**Host setup (`setup_host.yml`) and the shared engine build (`tasks/build_engine_hook.yml`):** both `setup_host.yml` and the manual `rebuild_minqlx.yml` delegate the actual build to the shared `tasks/build_engine_hook.yml`, which branches on the `runtime` extra-var (three `set_fact` blocks, one per runtime). minqlx compiles with the local patches under `ql-assets/patches/*.patch` applied first; vanilla minqlxtended applies no patches and links `minqlxtended.zip` (the Python package, zipped) beside the compiled `.so` instead of an unpacked package directory; minqlxtended-patched builds the `alexeytihomirov/minqlxtended` fork (upstream plus the former qlhub patch chain landed as commits: democut + native per-match demo capture + `demo_arm`/`demo_disarm` bindings, `set_position`) with no patch scripts at all — the old chain's item natives are covered by upstream v1.1.0's `item_touch`/`respawn_item`/writable entity fields, with stock fallbacks in the plugins themselves. The vendored chain under `ql-assets/patches/minqlxtended/` remains only as the rollback path (repoint the flavor's repo/version and re-enable `_engine_apply_minqlxtended_qlhub_patches`). All three builds run under the `ql` system user. `qlds@.service.j2` execs the wrapper by its actual per-runtime name via the `launch_script` extra-var (`ui.runtime.runtime_extravars()`), which every instance-level playbook (`add_qlds_instance.yml`, `sync_instance_configs_and_restart.yml`, `update_instance_hooks.yml`, `update_instance_lan_rate.yml`) already merges into its extra-vars before rendering the unit. Each build also syncs a runtime-appropriate `requirements.txt` from `ql-assets/data/<runtime_plugins_dirname>/`.

**System hooks:** minqlxtended (both variants) hooks `Sys_IsLANAddress` itself, unconditionally, in its own source (`src/server/hooks.c`). The built-in `force_rate.so` LD_PRELOAD hook overwrites the same function prologue and can crash the server if its `STATIC_SEARCH` fails, so `runtime_paths()['excluded_system_hooks']` excludes it on both minqlxtended variants — `_build_ld_preload_paths()` in `ui/task_logic/ansible_instance_mgmt.py` skips any system hook named in that set before it ever reaches LD_PRELOAD.

**Plugin baseline:** `ql-assets/data/<runtime>-plugins/` (`minqlx-plugins` or `minqlxtended-plugins`) holds every plugin that ships with a host of that runtime, independent of any preset. minqlxtended-patched uses `minqlx-plugins` (see the runtime table above), not a baseline of its own. Two delivery paths cover it, and they run in this order: `setup_host.yml` mirrors the directory onto the host once, at setup time (`{{ common_assets_dir }}/{{ runtime_plugins_dirname }}/`, `delete: yes`); `add_qlds_instance.yml` then rsyncs the preset's `scripts/` into the new instance's plugin directory (also `delete: yes` — an operator's selection is authoritative for what it lists), and only after that backfills anything from the host baseline the preset didn't already provide (`--checksum`, so a fix landed in the common pool after an instance's first backfill still reaches it). The baseline is therefore always present on every instance but never overrides a preset choice, and it is not something an operator picks — that's what the per-instance preset overlay (see **ConfigPreset Model** above) is for.
...
-   **Playbook Structure:**
    -   **`ansible/playbooks/setup_host.yml`:** Performs the initial one-time setup on a newly provisioned host. Installs prerequisites (including `iptables-persistent`, and `redis-server` only when the host runtime needs its own Redis), configures the firewall using a template (`ansible/templates/iptables.rules.j2`) that defines both filter and NAT rules which are applied atomically via `iptables-restore` and persisted, creates the `ql` user, installs base SteamCMD/QLDS and the host's selected runtime to its shared location (`/home/ql/qlds-base`, plus `/home/ql/minqlx-shared` or `/home/ql/minqlxtended-shared` — see the runtime table above), and syncs common assets (`/home/ql/assets/common`). The actual engine build is delegated to `tasks/build_engine_hook.yml` (see **Server Runtimes** above). Building either minqlxtended variant asserts the host's Python is 3.12+ before compiling.
    -   **`ansible/playbooks/rebuild_minqlx.yml`:** Manual-only playbook (not wired into the QLSM UI/task queue) that rebuilds and redeploys the shared engine build without a full host setup, via the same `tasks/build_engine_hook.yml`. Invoked directly with `-e '{"runtime": "..."}'`.
    ...
    -   **`ansible/playbooks/sync_instance_configs_and_restart.yml`:** Applies saved configuration changes to an existing instance. It syncs configs, factories, plugin drafts, and `user-hooks/` to the host, mirrors the whole shared runtime build (the engine `.so`, its launch script, and — for minqlx and minqlxtended-patched — the `minqlx`/`minqlxtended` Python package; vanilla minqlxtended ships `minqlxtended.zip` instead of an unpacked package) into the instance directory, re-renders the service unit with the current `LD_PRELOAD`, and restarts only when requested/required. Mirroring the whole directory rather than individual files is what lets a rebuilt runtime (see `setup_host.yml` and `rebuild_minqlx.yml` above) reach already-existing instances: the binary and the Python package/zip are two halves of the same build, and syncing only one half leaves patched event dispatchers unregistered at runtime. Compares by content (`--checksum`), not mere presence, so a fix landed in the common plugin pool still reaches an instance that was already backfilled once.
