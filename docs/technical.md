...
-   **CLI Modules (`ui/user_cli.py`, `ui/preset_cli.py`, `ui/builtin_presets.py`, `ui/plugin_repository_cli.py`):** Provide focused Flask CLI commands such as `flask create-user`, `flask create-default-admin`, and preset management commands including `flask sync-builtin-presets` (idempotently seeds built-in presets from the image into the database; run automatically by the Docker entrypoint on each container start), and `flask seed-plugin-repositories` (adds the default plugin repositories listed in `DEFAULT_PLUGIN_REPOSITORIES`, skipping any already present by name or URL; the entrypoint runs it only when it first initializes the database, so a repository the operator deletes is not re-added).
...
**Plugin baseline:** the shared plugin pool has two tiers per runtime, resolved through `ui/plugin_pool.py`. The **built-in tier** `ql-assets/data/<runtime>-plugins/` (`minqlx-plugins` or `minqlxtended-plugins`) holds every plugin that ships with a host of that runtime, independent of any preset; it is baked into the Docker image and only a release changes it. The **operator tier** `data/shared-plugins/<runtime>/` holds everything downloaded from a plugin repository. It lives under the `./data` bind mount on purpose: `ql-assets/` is image content, so a write there would land in the web container's own layer, invisible to the worker containers that ship plugins to hosts and gone on the next image pull. Every reader (`list_shared_plugins`, `resolve_pool_file`, `pool_file_hashes`, the manifest lookup in `ui/plugin_manifest.py`) sees one **merged view**: built-in files overlaid by operator files of the same name, so a download can deliberately shadow a bundled plugin (the download route requires `overwrite` for that, see the API reference). Two delivery paths cover the merged pool, and they run in this order: `setup_host.yml` mirrors it onto the host at setup time (`tasks/sync_common_plugins.yml` stages both tiers into one temp directory on the controller with `rsync -a`, operator tier last, then does a single `synchronize` with `delete: yes`, so a plugin removed from either tier disappears from the host too) (target `{{ common_assets_dir }}/{{ runtime_plugins_dirname }}/`; **Check for Updates** re-runs the same task on demand, see below); `add_qlds_instance.yml` then rsyncs the preset's `scripts/` into the new instance's plugin directory (also `delete: yes` — an operator's selection is authoritative for what it lists — except for plugin data files: `P_*.log`, `P_*.txt`, `P_*.json` and `P_*.db` protect rules keep what plugins such as `autokick` write next to their `.py` file from being deleted, while still pushing those files when the instance's `scripts/` has them), and only after that backfills anything from the host baseline the preset didn't already provide (`rsync --checksum`, excluding by name the files already present from `scripts/` so deliberate per-instance selections are never clobbered). The baseline is therefore always present on every instance but never overrides a preset choice. The config editor's Plugins tab lists its root `.py` files (excluding `__init__.py` and `SYSTEM_PLUGINS`) next to the draft's own files as `shared` rows (`ui/plugin_pool.py`, appended in `draft_routes.get_draft_tree` for `draft_effective_runtime`), so an operator can enable a baseline plugin the preset doesn't carry. Enabling one only adds its name to `qlx_plugins`, and the backfill delivers the file. Editing one writes a local copy into the draft (plain `PUT content`), which then wins on deploy.

**Push after download:** `ui/plugin_push.push_pool_to_hosts(runtimes)` is called by the repository download route once at least one file landed in the operator tier. For every host whose `runtime` is in the set and whose status is ACTIVE it takes the host lock (TTL 240 s, same as the apply endpoint) and enqueues `apply_plugin_updates_task(host_id, True, {}, [])`, i.e. a common-pool refresh and nothing else; hosts that are not ACTIVE, already locked, or unable to take the lock or enqueue (Redis down) are returned as skipped with a reason and never block the download: the files are already in the pool by then, so the push can only ever degrade to "run Check for Updates later". The Plugins tab closes the remaining gap on the frontend: `useHostPoolStatus` calls `GET /hosts/<id>/plugin-updates` when the tab opens for an ACTIVE host, keeps the `added` common-pool entries as the set of pool files the host is missing, and `SharedPluginBadges` marks those shared rows **not on host** with a push button that posts to the apply endpoint (`update_common_pool: true`) and polls the check for up to 90 s until the file appears, spacing ticks 5 s apart and queuing the next one only once the previous has returned (a check is a blocking SSH command, so a fixed interval would stack requests against the host being rsynced to). A check that answers 200 with `common_pool_error` set counts as unreadable rather than "nothing missing": the badges hide, the tab shows the unavailable note, and the poll keeps running to the deadline instead of reading as a completed push.

**Check for Updates (plugin pool refresh):** a host action that brings both delivery paths back in line with the merged pool without re-running host setup. Hashes come from `plugin_pool.pool_file_hashes(host_runtime(host))`, which overlays the operator tier on the built-in one, so a repository download shows up as **added** (or **modified**, when it shadows a bundled file) on the host's common pool until it is pushed. Instance file copies (`ansible_plugin_update._copy_selected_plugin_files`) resolve each name through `resolve_pool_file()` the same way.
...
- **Apply** (`POST …/plugin-updates/apply`, `ui/task_logic/ansible_plugin_update.py`, RQ task `apply_plugin_updates_task`, host lock):
  ...
  Nothing reaches a running instance until it restarts. `sync_instance_configs_and_restart.yml` mirrors `scripts/` into the instance plugin directory with `delete: yes` (same plugin-data protect rules as above), then backfills the host pool with `rsync --checksum` (excluding the instance's own `scripts/` filenames), so a refreshed pool file — including `SYSTEM_PLUGINS` such as `serverchecker.py` — replaces a stale instance copy. A newly copied file is still not loaded unless it is in the instance's `qlx_plugins` selection.
...
**Backups:** `ui/task_logic/backup_files.py` registers the operator tier (`data/shared-plugins/`, both runtimes) as the `plugins/shared-plugins` archive tree. The built-in tiers are image-owned and deliberately not captured, for the same reason `configs/presets/_builtin` is skipped: a restore must never replace a newer release's bundled plugins with an older copy. Archives written before the split carry `plugins/minqlx-plugins` and `plugins/minqlxtended-plugins` trees; restore only extracts prefixes listed in `backup_file_trees()`, so those are ignored.
