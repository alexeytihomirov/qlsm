...
**Operator Model:** Directory of named operators (`name`, `steam_id64` unique, `default_level` 0-5) assignable as Owner (`qlx_owner` in `server.cfg`) or Admin from the Owner & Admins tab (instance editors) or panel (preset pages). Managed via `operator_routes.py` at `/api/operators`. Admin levels live only in the instance's minqlx Redis permission keys; QLSM keeps no admin table. `ui/task_logic/permission_read.py` reads them for the tab's GET endpoint, and `ui/task_logic/access_permission_sync.py` writes only the changes from a config save (and the Add Instance list once after deploy). `access.txt` only carries Quake Live's own `admin`/`mod`/`ban` role lines; `ui/admin_permissions.py` strips any leftover numeric `steamid|level` line on save.
