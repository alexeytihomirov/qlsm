...
**Host setup (`setup_host.yml`) and the shared engine build (`tasks/build_engine_hook.yml`):** both `setup_host.yml` and the manual `rebuild_minqlx.yml` delegate the actual build to the shared `tasks/build_engine_hook.yml`, which branches on the `runtime` extra-var (three `set_fact` blocks, one per runtime). minqlx compiles with the local patches under `ql-assets/patches/*.patch` applied first; vanilla minqlxtended applies no patches and links `minqlxtended.zip` (the Python package, zipped) beside the compiled `.so` instead of an unpacked package directory; minqlxtended-patched additionally applies the 7-script qlhub patch chain vendored under `ql-assets/patches/minqlxtended/` (item events/respawn, native demo capture, a bounded redis pool, `set_position`) via `qlds-scripts/patch-minqlxtended-qlhub.py`, the same chain built and verified by `ql-server-core`'s own `build-minqlxtended` CI job. All three builds run under the `ql` system user. `qlds@.service.j2` execs the wrapper by its actual per-runtime name via the `launch_script` extra-var (`ui.runtime.runtime_extravars()`), which every instance-level playbook (`add_qlds_instance.yml`, `sync_instance_configs_and_restart.yml`, `update_instance_hooks.yml`, `update_instance_lan_rate.yml`) already merges into its extra-vars before rendering the unit. Each build also syncs a runtime-appropriate `requirements.txt` from `ql-assets/data/<runtime_plugins_dirname>/`.
