...
- **Apply** (`POST …/plugin-updates/apply`, `ui/task_logic/ansible_plugin_update.py`, RQ task `apply_plugin_updates_task`, host lock):
  ...
  Nothing reaches a running instance until it restarts. `sync_instance_configs_and_restart.yml` mirrors `scripts/` into the instance plugin directory with `delete: yes` (same plugin-data protect rules as above), then backfills the host pool with `--ignore-existing`, so a refreshed pool file replaces the instance's old copy at that point. A newly copied file is still not loaded unless it is in the instance's `qlx_plugins` selection.
