...
vi.mock('../../../services/api', () => ({
  ...
  getTelemetryRelay: vi.fn(),
  getTelemetryRelayStatus: vi.fn(),
  getHostStatsHubOverride: vi.fn(),
...
describe('resolveBundledComponent', () => {
  it('resolves a known component for a bundled addon', () => {
    const addon = { id: 'telemetry-relay', source: 'bundled' };
    const entry = { component: 'bundled:relay-modal' };
    expect(resolveBundledComponent(addon, entry)).toBe(
      BUNDLED_ADDON_COMPONENTS['telemetry-relay:relay-modal']);
  });

  it('refuses to hand a core component to an installed addon', () => {
    // An uploaded .zip must not be able to borrow core's build.
    const addon = { id: 'telemetry-relay', source: 'installed' };
    expect(resolveBundledComponent(addon, { component: 'bundled:relay-modal' })).toBeNull();
  });
...
describe('telemetry-relay bundled modal', () => {
  const Modal = BUNDLED_ADDON_COMPONENTS['telemetry-relay:relay-modal'];

  it('renders the same screen as the built-in one', async () => {
    addonRequest.mockResolvedValue({ enabled: false, url_override: '', ingest_token_override: '' });
    renderLazy(<Modal isOpen onClose={() => {}} entity={HOST} />);

    // The fields the operator had before, all of them.
    expect(await screen.findByText('Telemetry Relay', {}, LAZY_WAIT)).toBeInTheDocument();
    expect(screen.getByText('Sidecar Enabled')).toBeInTheDocument();
    expect(screen.getByText('Status')).toBeInTheDocument();
    expect(screen.getByText(/Stats Hub URL Override/i)).toBeInTheDocument();
    expect(screen.getByText(/Ingest Token Override/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /cancel/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /^save$/i })).toBeInTheDocument();
    // and it names the host, the way the built-in modal does
    expect(screen.getByText('finald')).toBeInTheDocument();
  });

  it('reads through the addon endpoints, not the core ones', async () => {
    addonRequest.mockResolvedValue({ enabled: true, url_override: 'https://hub', ingest_token_override: 't' });
    renderLazy(<Modal isOpen onClose={() => {}} entity={HOST} />);

    await waitFor(() => expect(addonRequest).toHaveBeenCalledWith(
      'telemetry-relay', 'GET', 'hosts/7'), LAZY_WAIT);
    expect(coreApi.getTelemetryRelay).not.toHaveBeenCalled();
    expect(coreApi.getHostStatsHubOverride).not.toHaveBeenCalled();
  });

  it('submits through the addon endpoint', async () => {
    addonRequest.mockResolvedValue({ enabled: false, url_override: '', ingest_token_override: '' });
    renderLazy(<Modal isOpen onClose={() => {}} entity={HOST} />);
    await screen.findByText('Sidecar Enabled', {}, LAZY_WAIT);

    await userEvent.click(screen.getByRole('button', { name: /^save$/i }));

    await waitFor(() => expect(addonRequest).toHaveBeenCalledWith(
      'telemetry-relay', 'PUT', 'hosts/7',
      { data: { enabled: false, url_override: '', ingest_token_override: '' } },
    ));
  });
});
