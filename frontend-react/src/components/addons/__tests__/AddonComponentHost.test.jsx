...
const ADDON = { id: 'sample-addon', name: 'Sample' };
...
describe('AddonComponentHost', () => {
  ...
  it('pins api to this addon and apiFor to whichever one it is given', async () => {
    ...
    ctx.api('GET', 'instances/7/items');
    expect(mocks.addonRequest).toHaveBeenLastCalledWith(
      'sample-addon', 'GET', 'instances/7/items', undefined,
    );

    // The cross-addon handle: a contributed action names the addon that owns
    // the route, and it must reach that addon, not this one.
    ctx.apiFor('other-addon')('POST', 'items/x/rebuild');
    expect(mocks.addonRequest).toHaveBeenLastCalledWith(
      'other-addon', 'POST', 'items/x/rebuild', undefined,
    );

    ctx.download('GET', 'items/download');
    expect(mocks.addonDownload).toHaveBeenLastCalledWith(
      'sample-addon', 'GET', 'items/download', undefined,
    );

    expect(mocks.addonAssetUrl).toHaveBeenLastCalledWith('sample-addon', 'ui/Panel.js');
  });
