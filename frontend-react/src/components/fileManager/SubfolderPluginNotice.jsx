 ...
 * actually be enabled (subfolder files, __init__.py). Those entries are dropped
 * from the checked set; this explains why so the change isn't silent.
 *
  ...
  return (
    <div role="status" className="alert-warning mb-3 flex items-start gap-2 text-sm">
      ...
      <span className="flex-1 min-w-0 text-[var(--text-secondary)]">
        ...
        Plugins in subfolders are referenced by plugins in the root folder, not enabled on their own.
      </span>
      <button
        type="button"
        onClick={onDismiss}
