...
export function isLanRateForcedOn(host) {
  const runtime = host && typeof host.runtime === 'string'
    ...
    : '';
  // Every runtime except plain minqlx is a minqlxtended engine build under
  // the hood (patched or not), and the 99k-only constraint is an engine
  // limitation the patches don't touch.
  return runtime !== '' && runtime !== 'minqlx';
}
