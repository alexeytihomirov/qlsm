...
QLSM_PLUGINS_BY_RUNTIME = {
    ...
    # This fork's minqlx-plugins pool is a different, smaller curated set than
    # minqlxtended-plugins' -- most of the seven above were never carried here
    # (verified against the pool directory at merge time with dngrtech/qlsm).
    # These are QLSM's own additions instead (match/telemetry/tournament
    # tooling, native demo capture, IRC bridging): not upstream minqlx-plugins
    # code, so a first-ever manifest entry for them must not default to
    # 'upstream'.
    'minqlx': {
        'serverchecker.py', 'match_restore.py', 'match_restore_lab.py',
        'match_restore_util.py', 'stream_telemetry_unified.py',
        'telemetry_unified_sched.py', 'stats_hub_pause.py',
        'tournament_access.py', 'chat_rcon.py', 'chat_rcon_acl.py',
        'demo_native_autorecord.py', 'demo_native_manifest.py', 'lobby.py',
    },
}
