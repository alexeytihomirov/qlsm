...
from ui.runtime import MINQLX, host_runtime
...
def lan_rate_forced_on(host):
    ...
    QLSM only ever configures minqlxtended instances (patched or not -- this is
    an engine-level constraint the patches don't touch) with sv_lanForceRate 1,
    so 25k is not a state it offers on either of those runtimes. The toggle
    itself is real on every runtime -- _build_qlds_args_string() has always
    emitted the cvar from the stored flag -- which is why the decision has to
    be enforced here.
    """
    return host_runtime(host) != MINQLX
