...
def apply_instance_config_logic(instance_id, restart=True, reconcile_lan_rate_network=False, previous_status=None, admin_levels=None):
    ...
    try:
        ...
        if runner_result.rc == 0:
            ...
            # of server.cfg react here -- e.g. an addon keeping its own
            # routing state in step with whatever cvars the file actually
            # carries, including when the operator set them by hand in the
            # Plugins tab and never used an addon's own assisted flow.
