"""Shared SFTP-to-a-managed-host plumbing for reading an instance's demo dir.

Split out of the demo-management addon (originally ansible_instance_demos.py)
once the qlmatch-packer addon needed the same "resolve instance -> host, open
a key-authenticated SFTP session, list/fetch named files" mechanics for its
own qlmatch-specific listing/download. Shared plumbing lives in core, the
addon keeps the domain logic that plumbing serves -- same precedent as
ui/stats_hub.py (see addons/README.md's "Where the line falls").
"""
import logging
import os
