...
def addon_app(tmp_path, monkeypatch):
    ...
    root = packages / 'telemetry-relay' / 'ui'
    root.mkdir(parents=True)
    (packages / 'telemetry-relay' / 'qlsm-addon.json').write_text(
        json.dumps(MANIFEST), encoding='utf-8')
    (packages / 'telemetry-relay' / 'backend.py').write_text(
        textwrap.dedent('''
            def register(ctx):
                pass
        '''), encoding='utf-8')
    (root / 'panel.js').write_text('export default () => null;', encoding='utf-8')
    (packages / 'telemetry-relay' / 'secrets.txt').write_text('do not serve me', encoding='utf-8')
...
def test_state_requires_auth(addon_app):
    assert addon_app.test_client().get('/api/addons/telemetry-relay/state').status_code == 401
...
def test_catalog_lists_the_installed_addon(addon_app, auth):
    ...
    assert [a['id'] for a in addons] == ['telemetry-relay']
    assert addons[0]['name'] == 'Telemetry Relay'
...
def test_state_defaults_before_anything_is_stored(addon_app, auth, host_id):
    resp = addon_app.test_client().get(
        f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}', headers=auth)
    assert resp.status_code == 200
...
def test_unknown_scope_is_400(addon_app, auth):
    resp = addon_app.test_client().get(
        '/api/addons/telemetry-relay/state?scope=galaxy', headers=auth)
    assert resp.status_code == 400


def test_non_integer_scope_id_is_400(addon_app, auth):
    resp = addon_app.test_client().get(
        '/api/addons/telemetry-relay/state?scope=host&scope_id=abc', headers=auth)
    assert resp.status_code == 400


def test_put_stores_settings(addon_app, auth, host_id):
    client = addon_app.test_client()
    resp = client.put(f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
                      json={'settings': {'timeout_sec': 9}}, headers=auth)
    assert resp.status_code == 200
    assert resp.get_json()['data']['settings']['timeout_sec'] == 9

    again = client.get(f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
                       headers=auth)
...
def test_put_rejects_an_out_of_range_value(addon_app, auth, host_id):
    resp = addon_app.test_client().put(
        f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
...
def test_rejected_write_leaves_nothing_behind(addon_app, auth, host_id):
    ...
    client = addon_app.test_client()
    client.put(f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
               json={'enabled': True, 'settings': {'timeout_sec': 999}}, headers=auth)
    state = client.get(f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
...
def test_effective_state_follows_the_layer_rule(addon_app, auth, host_id):
    ...
    client.put('/api/addons/telemetry-relay/state?scope=global', json={'enabled': True},
               headers=auth)
    state = client.get(f'/api/addons/telemetry-relay/state?scope=host&scope_id={host_id}',
                       headers=auth).get_json()['data']
...
def test_ui_asset_is_served(addon_app, auth):
    resp = addon_app.test_client().get('/api/addons/telemetry-relay/ui/panel.js', headers=auth)
    assert resp.status_code == 200
...
def test_non_js_asset_is_refused(addon_app, auth):
    ...
    resp = addon_app.test_client().get('/api/addons/telemetry-relay/ui/../secrets.txt',
                                       headers=auth)
    assert resp.status_code in (400, 403, 404)


def test_traversal_out_of_the_ui_dir_is_refused(addon_app, auth):
    resp = addon_app.test_client().get('/api/addons/telemetry-relay/ui/../backend.py',
                                       headers=auth)
    assert resp.status_code in (400, 403, 404)


def test_missing_asset_is_404(addon_app, auth):
    resp = addon_app.test_client().get('/api/addons/telemetry-relay/ui/nope.js', headers=auth)
    assert resp.status_code == 404
