"""The reference addon under addons/_examples/ui-kit-test-addon/ must actually work.

Same pattern as test_addon_css_test_addon.py, but this one pins the contract
that matters for the expanded window.__qlsm.ui kit: an addon's own tier-2
component can pull Modal/Button/Panel/AddonField/Icon/Stack/Row off it and
needs no CSS of its own to look consistent with the rest of QLSM.
"""
import os
import shutil
import tempfile
...
EXAMPLE_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), os.pardir, 'addons', '_examples', 'ui-kit-test-addon')
)
...
def test_example_manifest_validates_cleanly():
    ...
    assert manifest['id'] == 'ui-kit-test-addon'
...
def test_examples_directory_is_not_auto_loaded(app):
    with app.app_context():
        assert 'ui-kit-test-addon' not in registry.get_addons(app)
...
def installed_app(tmp_path, monkeypatch):
    ...
    packages.mkdir()
    shutil.copytree(EXAMPLE_DIR, packages / 'ui-kit-test-addon')
...
def test_example_addon_loads(installed_app):
    with installed_app.app_context():
        addon = registry.get_addon('ui-kit-test-addon', installed_app)
        assert addon is not None
        assert addon.loaded is True, addon.errors


def test_example_addon_serves_its_component(installed_app):
    """Reachable at the same asset route every tier-2 addon's component uses."""
    make_user(installed_app, 'operator', 'pw')
    auth = auth_headers(installed_app, 'operator')
    client = installed_app.test_client()

    js_resp = client.get('/api/addons/ui-kit-test-addon/ui/Panel.js', headers=auth)
