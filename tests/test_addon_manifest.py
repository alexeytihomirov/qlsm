...
def _minimal(**overrides):
    data = {'id': 'telemetry-relay', 'version': '1.0.0'}
...
def test_minimal_manifest_is_valid():
    ...
    assert errors == []
    assert manifest['id'] == 'telemetry-relay'
...
@pytest.mark.parametrize('bad_id', [
    'Telemetry',        # uppercase
    'telemetry_relay',  # underscore
    'telemetry--relay', # double dash
    '-telemetry',       # leading dash
    'telemetry-',       # trailing dash
    '',
])
...
def test_version_is_required():
    _, errors = validate_manifest({'id': 'telemetry-relay'})
...
def test_read_manifest_round_trip(tmp_path):
    (tmp_path / 'qlsm-addon.json').write_text(
        json.dumps(_minimal(name='Telemetry Relay')), encoding='utf-8'
    )
    manifest, errors = read_manifest(str(tmp_path))
    assert errors == []
