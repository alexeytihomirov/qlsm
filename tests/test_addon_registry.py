...
def test_installed_copy_overrides_the_bundled_one(addon_app):
    ...
    write_addon(bundled, 'telemetry-relay',
                manifest={'id': 'telemetry-relay', 'version': '1.0.0'})
    write_addon(packages, 'telemetry-relay',
                manifest={'id': 'telemetry-relay', 'version': '2.0.0'})
    app = build()
    addon = registry.get_addon('telemetry-relay', app)
...
def test_bundled_addon_is_used_when_nothing_overrides_it(addon_app):
    ...
    write_addon(bundled, 'telemetry-relay')
    app = build()
    assert registry.get_addon('telemetry-relay', app).source == 'bundled'
