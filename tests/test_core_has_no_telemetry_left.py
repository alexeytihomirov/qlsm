...
def test_the_telemetry_addon_still_provides_them():
    ...
    the separate qlsm-addons repo, installed by the operator onto the
    ADDON_PACKAGES_DIR volume) -- so a default app has nowhere to load it
    from and cannot assert its routes exist. demo-management is the one
...
def test_the_external_api_moved_with_qlmatch_packer():
    ...
    qlsm-addons repo (not bundled here), so a default app never loads it and
