"""Every declared lifecycle hook must actually be dispatched by core.
...
excluding the addon package itself). A precise call-graph analysis would be
more correct and far more brittle; what matters is that adding a name to
HOOK_SCOPES without wiring it fails the suite.
"""
import os
...
def _core_sources():
    """Every .py under ui/, except the addon package that defines the hooks."""
