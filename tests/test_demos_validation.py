...
plumbing (resolve instance -> host, open a session, list/fetch files) lives
in this addon's own instance_demo_transport.py; qlmatch-packer has its own
private copy of the same mechanics rather than sharing this one (see
tests/test_qlmatch_packer_listing.py and tests/test_addon_qlmatch_packer.py
for its own format-specific tests, and the external Bearer-token match API,
which moved there too).
